...
class Tool {
    ...
    */
	...
	public static function echo_relateknowledge($item,$num){
		//$conn = Conn::connect();
		$sql = "SELECT abrid,kl FROM `abbreviation` WHERE abrid IN (SELECT t.tabrid FROM (SELECT tabrid FROM `abbre_abbre` WHERE oabrid={$item['abrid']} LIMIT $num) AS t)";
		$result = Conn::select($sql);
		if(mysql_num_rows($result) > 0) {
			$row = mysql_fetch_array($result);
	...
	private static function _handle_wbmsg($row,$kl){
		...
		$str = $str." target='_blank'>{$row["sn"]}</a>: </span></div><div class='weibo_content'><span>";		 
	...
	public static function echo_weibobrief($item){
		//$conn = Conn::connect();
		$wbnum = 3;
		/*$sql = "SELECT mid,uid,sn,murl,mc,pt FROM `weibomsg` WHERE mid IN (SELECT t.mid FROM " 
		        . "(SELECT mid FROM `abbre_weibomsg` WHERE abrid={$item['abrid']} AND mid IN ("
		        . "SELECT mid FROM weibomsg ".") LIMIT $wbnum) AS t)";*/
		...
		$sql = "SELECT mid,uid,sn,murl,mc,pt FROM `weibomsg` WHERE uid IN ({$fui}) AND mid IN (SELECT mid FROM `abbre_weibomsg`"
		        . " WHERE abrid=" . $item['abrid'] . ") GROUP BY mid LIMIT $wbnum";
		$result = Conn::select($sql);
		if(mysql_num_rows($result) > 0) {
			$row = mysql_fetch_array($result);
	...
	public static function echo_wbmsg($row,$kl){
		/*$bpos = Tool::UTF8_stripos($row['mc'],$kl);
		if($bpos){
			$str = Tool::UTF8_substr($row['mc'],0,$bpos);
			$str = $str."<font color='#FF0000'>".$kl."</font>";
			$str = $str.Tool::UTF8_substr($row['mc'],stripos($row['mc'],$kl)+strlen($kl),Tool::UTF8_length($row['mc'])-Tool::UTF8_length($kl));
		} else {
			$str = $row['mc'];
		}*/
